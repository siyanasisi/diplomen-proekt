...
type HomeEventModalProps = {
    home: HomeState;
};

export function HomeEventModal({ home }: HomeEventModalProps) {
    const {
        role,
        studyPlan,
        selectedDay,
    ...
    const hasStudyTopics = Boolean(studyDay && studyDay.topics.length > 0);
                    ...
                    <div className="mb-6 space-y-4">
                        <div className="space-y-3">
                            {studyDay.topics.map((topic, idx) => (
                                ...
                                >
                                    <span
                                        className={`text-xs font-semibold px-2 py-1 rounded ${topic.subject === "Български език" ? "bg-purple-200 text-purple-700" : "bg-amber-200 text-amber-700"}`}
                                    >
                                        {topic.subject}
                                    </span>
